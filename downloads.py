...
def manage(tc, complete_dir, out_dir, extract_dir, dry_run=False, delete=True):
    ...
    to_remove.extend([c for c in possibles if
                      (c['torrent'] and c['torrent'].status == "stopped" and c['torrent'].error == 0) and not c[
                          'has_rars']])
    ...
    to_extract_and_remove.extend([c for c in possibles if
                                  (c['torrent'] and c['torrent'].status == "stopped" and c['torrent'].error == 0) and c[
                                      'has_rars']])
